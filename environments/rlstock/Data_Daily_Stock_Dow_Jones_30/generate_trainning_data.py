...
def generate_data(train_data=train_data, windows = 50, filename=abs_rlstock_env_window):
    ...
    for j in trange(len(Date)):
        ...
        if date >= Date[windows-1]:
            ...
            for i in range(len(select_stocks_list)):
                ...
                obs_one = temp_tics_data.Close.iloc[iloc] - np.array(temp_tics_data.iloc[iloc-windows+1:iloc+1,-3:])
