...
class sac_discrete_class:
    ...
    def test_agent(self):
        for j in range(self.num_test_episodes):
            o, d, ep_ret, ep_len = self.test_env.reset(isRandomStart=True), False, 0, 0
            while not (ep_len == self.max_ep_len):
                ...
                with torch.no_grad():
                    if self.state_of_art_model and o.ndim == 2:
                        obs = torch.FloatTensor(o).view([1, 1, *self.obs_dim]).to(self.device)
                    else:
                        obs = torch.FloatTensor(o).view([1, *self.obs_dim]).to(self.device)
                ...
                if d == 1:  # 资金不足
                    ...
                    self.test_env.reset(isRandomStart=True, total=self.test_env.current_env.total)
                elif d == 3:  # 达到回撤限制
                    print('test达到回撤限制')
                    break
    ...
    def run(self):
        ...
        while t < total_steps:
            ...
            if d == 1 or (ep_len == self.max_ep_len):  # ep_len == max_ep_len是游戏成功时最少ep长度
                o, ep_ret, ep_len = self.env.reset(isRandomStart=False), 0, 0
                eps += 1
            elif d == 2:  # 达到索引终点
                self.env.reset(isRandomStart=False, total=self.env.current_env.total)  #继续下一个合约,但是继承上一次总资产
            ...
            if (t + 1) % self.steps_per_epoch == 0 and self.replay_buffer.size > self.update_after:
...
def fgym_trunk_sac_discrete_v2(**kwargs):
